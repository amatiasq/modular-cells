...
export function accessor(getter, setter = `${getter} = #`, {
  log = false,
  logGet = log,
  logSet = log,
  debug = false,
  ...
  return (prototype, name) => {
    ...
    const set: any = new Function('value', `
      ${debugSet ? DEBUGGER : ''}
      ${logSet ? `with (this) console.log('SET', ${propSet}, value, 'PREV', ${setterCode})` : '' }
...
export function getter(code, {
  debug = false,
  debugGet = debug || false,
} = {}) {
  const get: any = new Function(`${debugGet ? DEBUGGER : ''} with(this) return ${code}`);
